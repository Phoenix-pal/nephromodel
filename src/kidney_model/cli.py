...
from .diagnostics import continuity_and_mass, physiology_metrics, state_summary
...
def main(argv=None):
    ...
    parser.add_argument("--residual-tol", type=float, default=1e-6, help="maximum accepted nonlinear residual")
    ...
    if args.dynamic_file:
        ...
        if args.best_seed:
            ...
            selected_row = select_best_dynamic_row(args.dynamic_file)
            print(f"selected best seed row={selected_row}")
        dynamic_state = load_dynamic_state(args.dynamic_file, selected_row)
    ...
    print("physiology:", physiology_metrics(result["final_state"], p))
